  ...
  <div class="container">
        ...
        <template #right>
          <el-form :inline="true" :model="filter" class="search-filter-form">
            <el-form-item label="状态">
              <el-select v-model="filter.status" clearable placeholder="请选择平台状态">
    ...
    <div class="card mt">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            {{ platformStatus(scope.row.platformStatus) }}
          </template>
        </el-table-column>
        <el-table-column
            prop="status"
      ...
      <el-dialog
          ...
          width="30%"
          center>
        <el-form ref="form" :rules="rules" :model="tempData" label-width="auto">
          <el-form-item label="平台名称" prop="title">
            <el-input v-model="tempData.title"></el-input>
          </el-form-item>
          <el-form-item label="开发者ID(AppID)" prop="appid">
            <el-input v-model="tempData.appid"></el-input>
          </el-form-item>
          <el-form-item label="开发者密码(AppSecret)" prop="secret">
            <el-input v-model="tempData.secret"></el-input>
          </el-form-item>
          <el-form-item label="类型" prop="type">
            <el-select v-model="tempData.type">
              <el-option value="wx" label="微信公众平台"/>
...
import {deletePlatform, getPlatform, getPlatformListPage, savePlatform, updatePlatformInfo} from "@/api/platform";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        appid: [
          {required: true, message: "开发者ID(AppID)不能为空", trigger: "blur"}
        ],
        secret: [
          {required: true, message: "开发者密码(AppSecret)不能为空", trigger: "blur"}
        ],
        type: [
  ...
  methods: {
    async getList() {
      this.loading = true
      let res = await getPlatformListPage({current: this.current, size: this.size, ...this.filter});
      const page = res.data;
    ...
    async editItem(id) {
      this.tempData = {};
      let res = await getPlatform(id);
      let {authenticationData, ...data} = res.data;
      this.tempData = {...data, ...JSON.parse(authenticationData)}
      this.$nextTick(() => {
        this.isDialogOpen = true;
      ...
      this.$refs.form.validate(validate => {
        ...
        if (validate) {
          ...
          let data = {
            ...
            platformStatus,
            isAuthentication,
            authenticationData: JSON.stringify(other)
          }
          if (data.id) {
            // 修改
            updatePlatformInfo(data).then(() => {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm('确认要' + text + row.title + '吗？').then(function () {
        return updatePlatformInfo({id: row.id, status: row.status});
      }).then(() => {
    ...
    handleBatchDelete() {
      this.$confirm('确定要删除这些平台吗？', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'danger'
      }).then(() => {
        let ids = this.checkList.map(item => item.id);
        deletePlatform(JSON.stringify(ids)).then(async () => {
          this.$message.success("删除成功");
          await this.getList();
